   ...
   perform  dm.update_at();
    ...
    if not exists(select 1 from dm.mis_overall_stat_dm where msr_prd_id = m_msr_prd_id limit 1) then
        insert into dm.mis_overall_stat_dm(msr_prd_id, ind_tp_id, mis_new, mis_all, mis_se, mis_s, mis_e, mis_t)
            (select m_msr_prd_id                                                                                 msr_prd_id,
                    ...
                    count(distinct formal_task_id) filter ( where create_msr_prd.msr_prd_id = m_msr_prd_id)                            mis_new,
                    count(distinct formal_task_id) filter ( where ft.start_date < m_msr_prd_eff_dt and ft.status <> 'FINISHED' or
                                            create_msr_prd.msr_prd_id = m_msr_prd_id )                           mis_all,
                    count(distinct formal_task_id) filter ( where end_msr_prd.msr_prd_id = m_msr_prd_id and ft.status = 'FINISHED')    mis_se,
                    count(distinct formal_task_id) filter ( where start_msr_prd.msr_prd_id = m_msr_prd_id and ft.status <> 'FINISHED') mis_s,
                    count(distinct formal_task_id) filter ( where ft.start_date < m_msr_prd_eff_dt  and end_msr_prd.msr_prd_id=m_msr_prd_id and ft.status = 'FINISHED')         mis_e,
                    count(distinct formal_task_id) filter ( where ft.start_date < m_msr_prd_eff_dt and ft.status <> 'FINISHED')         mis_t
             ...
             from portal.formal_tasks ft
                      ...
                      left join dm.msr_prd start_msr_prd on ft.start_date >= start_msr_prd.eff_dttm and
                                                       ft.start_date < start_msr_prd.end_dttm and
                                                       start_msr_prd.msr_prd_tp_id = m_msr_prd_tp_id
                      left join dm.msr_prd end_msr_prd
    ...
    if not exists(select 1 from dm.ev_overall_stat_dm where msr_prd_id = m_msr_prd_id limit 1) then
        ...
        select m_msr_prd_id,
               ...
               count(*) filter ( where ar_data.start_date < m_msr_prd_eff_dt  and end_msr_prd.msr_prd_id=m_msr_prd_id and  ar_data.status = 'FINISHED')    ev_e,
        ...
        from (
                 ...
                 join dm.msr_prd create_msr_prd on ar_data.creation_date >= create_msr_prd.eff_dttm and
                                                   ...
                                                   create_msr_prd.msr_prd_tp_id = m_msr_prd_tp_id
                 left join dm.msr_prd start_msr_prd on ar_data.start_date >= start_msr_prd.eff_dttm and
    ...
    if not exists(select 1 from dm.test_overall_stat_dm where msr_prd_id = m_msr_prd_id limit 1) then
        insert into dm.test_overall_stat_dm(msr_prd_id, ind_tp_id, test_cnt_all, test_cnt_url, test_cnt_d, test_cnt_dm,
            ...
            (select m_msr_prd_id,
                    case when at.access_tool_tp_id = 1 then 1 when at.access_tool_tp_id = 2 then 2 else 3 end     ind_tp_id,
                     count(*)                                                                                      test_cnt_all,
                    count(*) filter ( where ci.irtz_type = 'url')                                        as test_cnt_url,
                    count(*) filter ( where ci.irtz_type = 'domain')                                     as test_cnt_d,
                    count(*) filter ( where ci.irtz_type = 'domain_mask')                                as test_cnt_dm,
                    count(*)
                    filter ( where ci.irtz_type='ip')               as test_cnt_ip,
                    count(*) filter ( where ci.irtz_type='subnet')                               as test_cnt_sn
             from results.results ar
                      ...
                      join dm.access_tool at on robots.orig_id = at.orig_id and a.creation_date >= at.eff_dttm and
                       ...
                       join sor.content_history ch on ar.content_id=ch.content_id and ar.start_date>=ch.st_dt and ar.start_date<ch.end_dt
                      join sor.content_info  ci on ci.content_id=ar.content_id and ch.content_version_id=ci.content_version_id
