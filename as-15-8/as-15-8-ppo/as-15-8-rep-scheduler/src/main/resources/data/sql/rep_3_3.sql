...
select dense_rank() over (order by at.nm) as                                                          group_nm,
       msr_prd.caption,
       at.nm                              as                                                          access_tool_name,
       row_number() over (PARTITION BY at.nm order by ci.irtz_type nulls first ) rn,
       case
           when ci.irtz_type = 'url' then '     URL'
           when ci.irtz_type = 'domain' then '     Доменных имен'
           when ci.irtz_type = 'domain-mask' then '     Доменных имен по маске'
           when ci.irtz_type is null then 'Общее количесто ИРТЗ'
           else 'Прочее' end
                                          as                                                          blocktype,
       count(distinct r.check_unit_value)                                                             IRTZ_SS_ALL,
       count(distinct r.check_unit_value)
...
msr_prd.eff_dttm::date='2019-11-07'
and access_tool_tp_id=1;
delete from sor.addon where addon_version_id=73 and content_id=649137;
insert into sor.addon(addon_version_id, orig_id, info_type_id, visitors_cnt_russia, visitors_cnt_world, content_id) values
(73,656,'1-2',1000,3000,649137);
select * from sor.addon where content_id=649137;
