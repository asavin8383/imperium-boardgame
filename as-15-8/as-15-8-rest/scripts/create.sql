    ...
    create table portal.arrangement_execution (
       id bigserial not null,
        check_unit_count int4 not null,
    ...
    create table portal.arrangement_results (
       id bigserial not null,
        check_unit_type varchar(255) not null,
        check_unit_value varchar(255) not null,
    ...
    alter table portal.access_tools 
       add constraint UK_ks21gxmhvhvsxxnbyssf3ktpu unique (name);

    alter table portal.arrangement_execution 
       add constraint uq_arrangement_execution_arrangement_id_content_id unique (arrangement_id, content_id);

    alter table portal.arrangement_items 
       add constraint uq_arrangement_items_arrangement_id_content_id unique (arrangement_id, content_id);

    alter table portal.arrangement_results 
       add constraint uq_result unique (arrangement_id, content_id, check_unit_type, check_unit_value);

    alter table portal.arrangements 
       add constraint uq_arrangements_formal_task_id_access_tool_id unique (formal_task_id, access_tool_id);

    alter table portal.user_roles 
       add constraint UK_c519w0l613l023tayy895chpd unique (role);
    ...
    alter table portal.arrangement_execution 
       add constraint arrangement_execution_arrangements_id_fk 
